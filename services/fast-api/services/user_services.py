...
def get_users_service(skip: int, limit: int) -> Dict[str, Any]:
    info_logger.info(
        "Fetching all users | skip=%s limit=%s", skip, limit,
    )
    try:
        ...
        error_logger.exception(
            f"Database error while fetching users: {e.args}"
        )
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        error_logger.exception(
            f"Unexpected error in get_users_service: {e.args}",
        )
...
def get_user_by_id_service(user_id: int) -> Dict[str, Any]:
    ...
    try:
        ...
        error_logger.exception(
            f"Database error while fetching user by id: {e.args}"
        )
        raise HTTPException(status_code=500, detail="Database error")
    except Exception as e:
        error_logger.exception(
            f"Unexpected error in get_user_by_id_service: {e.args}",
        )
        raise HTTPException(status_code=500, detail="Internal server error")